...
class _OfferCardState extends State<OfferCard> {
  ...
  bool isRelatedRideFinished = false;
  bool isRelatedRideCancelled = false;
  String inactiveOfferLabel = "";
  ...
  void didUpdateWidget(covariant OfferCard oldWidget) {
    ...
    if (oldWidget.offer.id != widget.offer.id ||
        oldWidget.offer.availableSeats != widget.offer.availableSeats ||
  ...
  Future<void> checkIfRequested() async {
    ...
    setState(() {
      ...
      if (status == "0" || status.contains("pending")) {
        ...
        // Keep true if you want rejected users to request again.
  ...
  Future<void> checkRelatedRideStatus() async {
    if (widget.offer.id == null) {
      return;
    }

    final rides = await RideUtils.getRides();

    final relatedRides = rides
        .where((ride) => ride.offerId.toString() == widget.offer.id.toString())
        .toList();

    if (!mounted) return;

    if (relatedRides.isEmpty) {
      setState(() {
        isRelatedRideFinished = false;
        isRelatedRideCancelled = false;
        inactiveOfferLabel = "";
      });
      return;
    }

    final relatedRide = relatedRides.first;
    final status = relatedRide.status.toString().toLowerCase();

    final bool finished = status.contains("finished") ||
        status.contains("completed") ||
        status == "3";

    final bool cancelled = status.contains("cancelled") || status == "4";

    setState(() {
      isRelatedRideFinished = finished;
      isRelatedRideCancelled = cancelled;

      if (finished) {
        inactiveOfferLabel = "Пътуването е завършено";
      } else if (cancelled) {
        inactiveOfferLabel = "Пътуването е отменено";
      } else {
        inactiveOfferLabel = "";
      }
    });
  ...
  String formatDateTime(String dateTimeStr) {
    ...
    return "${dt.day}-${dt.month}-${dt.year} в ${dt.hour}:$minuteStr";
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool isOfferInactive =
        isRelatedRideFinished || isRelatedRideCancelled;

    final bool shouldDisableRequestButton = isOfferInactive ||
        offer.availableSeats <= 0 ||
        (isOfferRequested && !canRequestAgain);

    return Opacity(
      opacity: isOfferInactive ? 0.58 : 1,
      child: Card(
        color: isOfferInactive ? Colors.grey.shade100 : Colors.white,
        elevation: isOfferInactive ? 0 : 3,
        margin: const EdgeInsets.symmetric(vertical: 8.0),
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(15),
          side: isOfferInactive
          ...
          child: Column(
            ...
            children: [
              ...
              if (isOfferInactive) ...[
                ...
                buildInactiveOfferLabel(),
              ],

              const Divider(height: 24),

              buildOfferInfoAndAction(
                offer,
                shouldDisableRequestButton,
              ),
  ...
  Widget buildInactiveOfferLabel() {
    final bool isFinished = isRelatedRideFinished;
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            isFinished ? Icons.check_circle_outline : Icons.cancel_outlined,
            size: 18,
            color: isFinished ? Colors.green : Colors.red,
          ),
  ...
  Widget buildOfferInfoAndAction(
    Offer offer,
    bool shouldDisableRequestButton,
  ) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget buildSeatsRow(Offer offer) {
    final bool noSeats = offer.availableSeats <= 0;
    ...
    return Row(
      children: [
        Icon(
          Icons.event_seat,
          size: 16,
          color: noSeats ? Colors.red : Colors.grey,
        ),
        const SizedBox(width: 4),
        Text(
          noSeats ? "Няма свободни места" : "${offer.availableSeats} свободни места",
          style: TextStyle(
            color: noSeats ? Colors.red : Colors.grey,
            fontWeight: noSeats ? FontWeight.bold : FontWeight.normal,
          ),
        ),
      ],
    );
  ...
  Widget buildRequestButton(Offer offer, bool shouldDisableRequestButton) {
    ...
    if (isRelatedRideFinished || isRelatedRideCancelled) {
      buttonText = "Неактивна";
    } else if (offer.availableSeats <= 0) {
      buttonText = "Няма места";
    } else if (isOfferRequested && !canRequestAgain) {
      buttonText = "Заявено";
  ...
  Widget buildEditButton(Offer offer) {
    ...
    return ElevatedButton.icon(
      ...
      label: Text(isOfferInactive ? "Неактивна" : "Редактирай"),
      style: ElevatedButton.styleFrom(
