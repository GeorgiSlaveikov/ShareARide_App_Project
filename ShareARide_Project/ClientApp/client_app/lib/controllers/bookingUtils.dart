...
class BookingUtils {
  ...
  static Future<List<Booking>> getBookings() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        var bookingsList = (bookings as List).map((booking) => Booking.fromJson(booking)).toList();
        return bookingsList;
      }
    } catch (e) {
  ...
  static Future<Booking> getBooking(int id) async {
    ...
    try {
      ...
      print("API unreachable: $e");
      return Booking(requestedForId: 0, requestorId: 0, offerId: 0, passengers: []);
    }
    return Booking(requestedForId: 0, requestorId: 0, offerId: 0, passengers: []);
  }
  ...
  static Future<List<Booking>> getBookingsForMe(int id) async {
    final url = Uri.parse('http://${Utils().ip}:5205/api/bookings/requests_for_me/$id');
